...
1. What problem does the context API help solve?
  ...
  The context API allows a developer to add to the state data that will only be available in one branch of a data tree, or where ever the app is wrapped at. 

2. In your own words, describe `actions`, `reducers` and the `store` and their role in Redux. What does each piece do? Why is the store known as a 'single source of truth' in a redux application?

  Packets of information (Action Creators, Action Types) => Tranisitions to the next state => Initial State/Current State


3. What is the difference between Application state and Component state? When would be a good time to use one over the other?

  Application State is like Global State and Component State is like Local State. If a component is only going to be used in one branch of the data tree, component state is probably better because then it won't be possible to accidentally screw with the data somewhere else in your application. 


4. Describe `redux-thunk`, what does it allow us to do? How does it change our `action-creators`?

  Redux Thunk is middleware. The Thunk Middleware lets you write async logic that interacts with the store.

5. What is your favorite state management system you've learned and this sprint? Please explain why!

  Redux. I really like the idea of being able to play around with the same data all over the application in a project that would be bigger scale. Being able to know what's available to you at all times. 
